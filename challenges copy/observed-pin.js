...
function getPINs(observed) {
  ...
  const firstChar = observed.slice(0, 1);
  let remainingChars = observed.slice(1);
  console.log(firstChar, remainingChars);
  function concatAdjVals(arr = adjacentVals[firstChar], str = observed.slice(1)) {
    if (!str.length) return arr;
    arr = arr.reduce((acc, combo) => {
      let result = [];
      for (let i = 0; i < adjacentVals[str[0]].length; i += 1) {
        const elt = adjacentVals[str[0]][i];
        result.push(combo + elt);
      }
      return acc.concat(result);
    }, []);
    return concatAdjVals(arr, str.slice(1));
  }
  return concatAdjVals();
}
